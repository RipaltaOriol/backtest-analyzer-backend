...
def get_file(setup_id):
    ...
    reportlab.rl_config.TTFSearchPath.append(
        os.path.join(app.root_path, app.config["FONTS_FOLDER"], "Montserrat/static")
    )
    ...
    style.add(
        ParagraphStyle(
            name="date", alignment=TA_RIGHT, fontSize=11, fontName="Montserrat"
        )
    )
    style.add(
        ParagraphStyle(
            name="subtitle", parent=style["Normal"], fontName="Montserrat", fontSize=11
        )
    )
    style.add(
        ParagraphStyle(
    ...
    a4_frame = Frame(
        doc.leftMargin, doc.bottomMargin, doc.width, doc.height, id="a4_frame"
    )
    ...
    p, tx, autotexts = plt.pie(
        pie_data, labels=["Wins", "Break-Even", "Loss"], autopct=""
    )
    ...
    doc.addPageTemplates(
        [
            ...
            PageTemplate(
                id="landscape", frames=landscape_frame, pagesize=landscape(A4)
            ),
        ]
    )
    ...
    return send_file(
        ...
        attachment_filename="booking.pdf",
        cache_timeout=0,
    )
